...
export default function RequestReviewPage() {
  ...
  async function refreshAutoReply(
    ...
    setSendError("")
    try {
      const nextReply = await requestReply(intent, nextTone, nextCost)
  ...
  async function updateStatus(status: string) {
    ...
    setSendError("")
    let replyForStatus = reply

    if (status === "declined") {
      setGeneratingReply(true)
      try {
        const generatedReply = await requestReply("decline", tone, { costMin, costMax })
        if (generatedReply === null) return

        replyForStatus = generatedReply
        setReply(generatedReply)
        setReplyTouched(true)
        setReplyAutoGenerated(true)
        checkAccidentalYes(generatedReply)
      } finally {
        setGeneratingReply(false)
      }
    }
